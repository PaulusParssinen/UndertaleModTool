...
public class UndertaleData : IDisposable
{
    ...
    /// <exception cref="NotSupportedException"> if the type is not an <see cref="IUndertaleNamedResource"/>.</exception>
    ...
    public IList this[Type resourceType]
    {
        ...
        {
            // Prevent recursion
            if (resourceType == typeof(IUndertaleResource))
                return null;

            if (!typeof(IUndertaleResource).IsAssignableFrom(resourceType))
    ...
    /// <returns>The <see cref="IUndertaleResource"/>.</returns>
    public IUndertaleNamedResource ByName(string name, bool ignoreCase = false)
    {
        ...
        return Sounds.ByName(name, ignoreCase) ??
            ...
            (IUndertaleNamedResource)null;
    }

    /// <summary>
    /// Reports the zero-based index of the first occurence of the specified <see cref="IUndertaleResource"/>.
    /// </summary>
    ...
    public int IndexOf(IUndertaleResource obj, bool panicIfInvalid = true)
    {
